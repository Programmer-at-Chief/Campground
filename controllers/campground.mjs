...
const create_campground = async (req,res,next) => {
  try{
    ...
    const campground = new Campground({title: title, city: city,state: state,description: description,price : Number(price),author : res.locals.currentUser._id,geometry : {type : 'Point',coordinates : [25.000000, -71.000000]}})
    ...
    const output= await radar.get_coordinates(`${city}, ${state}`)
